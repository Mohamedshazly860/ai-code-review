      ...
      <div style={{ width: '100%', maxWidth: '400px' }}>
        ...
        <div style={{ background: '#252526', border: '1px solid #3C3C3C', borderRadius: '10px', padding: '2rem' }}>
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <Input label="Username" type="text" placeholder="cool_dev_42"
              value={form.username} error={errors.username}
              onChange={e => setForm(p => ({ ...p, username: e.target.value }))} />
            <Input label="Email" type="email" placeholder="dev@example.com"
        ...
        <p style={{ textAlign: 'center', marginTop: '1.25rem', fontSize: '0.85rem', color: '#6A6A6A' }}>
          ...
          <Link to="/login" style={{ color: '#569CD6', textDecoration: 'none' }}>Sign in</Link>
        </p>
      </div>
    </div>
  )
